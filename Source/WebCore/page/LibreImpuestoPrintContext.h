...
/* Patched by Henry Castro <hcvcastro@gmail.com> Marzo, 2014 */

#ifndef LibreImpuestoPrintContext_h
#define LibreImpuestoPrintContext_h
...
class LibreImpuestoPrintContext {
public:
  LibreImpuestoPrintContext(Frame*, Frame*, Frame*);
    ~LibreImpuestoPrintContext();

    Frame* frame() const { return m_contentFrame; }
    Frame* headerFrame() const { return m_headerFrame; }
    Frame* footerFrame() const { return m_footerFrame; }
    ...
    size_t pageCount() const { return m_pageContentRects.size(); }
    const IntRect& pageRect(size_t pageNumber) const { return m_pageContentRects[pageNumber]; }
    const Vector<IntRect>& pageRects() const { return m_pageContentRects; }
